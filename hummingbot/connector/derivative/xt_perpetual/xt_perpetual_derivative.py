...
import hummingbot.connector.derivative.xt_perpetual.xt_perpetual_constants as CONSTANTS
import hummingbot.connector.derivative.xt_perpetual.xt_perpetual_utils as xt_utils
from hummingbot.connector.derivative.position import Position
from hummingbot.connector.derivative.xt_perpetual import xt_perpetual_web_utils as web_utils
from hummingbot.connector.derivative.xt_perpetual.xt_perpetual_api_order_book_data_source import (
    XtPerpetualAPIOrderBookDataSource,
)
from hummingbot.connector.derivative.xt_perpetual.xt_perpetual_auth import XtPerpetualAuth
from hummingbot.connector.derivative.xt_perpetual.xt_perpetual_user_stream_data_source import (
    XtPerpetualUserStreamDataSource,
)
...
class XtPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    def __init__(
        ...
        xt_perpetual_api_key: str = None,
        xt_perpetual_secret_key: str = None,
        trading_pairs: Optional[List[str]] = None,
        trading_required: bool = True,
        domain: str = CONSTANTS.DEFAULT_DOMAIN,
    ):

        self.xt_perpetual_api_key = xt_perpetual_api_key
        self.xt_perpetual_secret_key = xt_perpetual_secret_key
    ...
    @property
    def authenticator(self) -> XtPerpetualAuth:
        return XtPerpetualAuth(self.xt_perpetual_api_key, self.xt_perpetual_secret_key)
    ...
    def supported_position_modes(self) -> List[PositionMode]:
        if all(xt_utils.is_linear_perpetual(tp) for tp in self._trading_pairs):
            return [PositionMode.ONEWAY, PositionMode.HEDGE]
        elif all(not xt_utils.is_linear_perpetual(tp) for tp in self._trading_pairs):
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return XtPerpetualAPIOrderBookDataSource(
            self.trading_pairs,
            connector=self,
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return XtPerpetualUserStreamDataSource(
            auth=self._auth,
            api_factory=self._web_assistants_factory,
            domain=self._domain,
        )
    ...
    async def _update_positions(self):
        ...
        for position in parsed_resps:
            ...
            leverage = Decimal(str(data["leverage"])) if xt_utils.is_linear_perpetual(hb_trading_pair) \
    ...
    async def _all_trade_updates_for_order(self, order: InFlightOrder) -> List[TradeUpdate]:
        ...
        if order.exchange_order_id is not None:
            try:
                ...
                trades_list_key = "data" if xt_utils.is_linear_perpetual(order.trading_pair) else "trade_list"
    ...
    async def _process_account_position_event(self, position_msg: Dict[str, Any]):
        ...
        # Trigger balance update because Xt doesn't have balance updates through the websocket
        ...
        for instrument in instrument_info_dict["result"]:
            try:
                ...
                if exchange_symbol in symbol_map:
                    ...
                    is_linear = xt_utils.is_linear_perpetual(trading_pair)
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(xt_utils.is_exchange_information_valid, exchange_info["result"]):
    ...
    async def _set_trading_pair_leverage(self, trading_pair: str, leverage: int) -> Tuple[bool, str]:
        ...
        if xt_utils.is_linear_perpetual(trading_pair):
            data = {
                "symbol": exchange_symbol,
