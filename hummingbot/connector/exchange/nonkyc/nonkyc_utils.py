...
async def api_call_with_retries(
    method, endpoint, params: Optional[Dict[str, Any]] = None, shared_client=None, try_count: int = 0
) -> Dict[str, Any]:
    ...
    if request_errors or parsed_response is None:
        if try_count < Constants.API_MAX_RETRIES:
            ...
            print(f"Error fetching data from {url}. HTTP status is {http_status}. " f"Retrying in {time_sleep:.0f}s.")
