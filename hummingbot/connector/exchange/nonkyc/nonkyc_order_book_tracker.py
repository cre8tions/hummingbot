                ...
                if message.type is OrderBookMessageType.DIFF:
                    ...
                    if len(mbids) > 0:
                        if isinstance(mbids[0], list):
                            np_bids = np.array(
                                [[message.timestamp, float(bid[0]), float(bid[1]), message.update_id] for bid in mbids],
                                dtype="float64",
                                ndmin=2,
                            )
                        else:
                            np_bids = np.array(
                                [
                                    [message.timestamp, float(bid["price"]), float(bid["quantity"]), message.update_id]
                                    for bid in mbids
                                ],
                                dtype="float64",
                                ndmin=2,
                    ...
                    if len(masks) > 0:
                        if isinstance(mbids[0], list):
                            np_asks = np.array(
                                [[message.timestamp, float(ask[0]), float(ask[1]), message.update_id] for ask in masks],
                                dtype="float64",
                                ndmin=2,
                            )
                        else:
                            np_asks = np.array(
                                [
                                    [message.timestamp, float(ask["price"]), float(ask["quantity"]), message.update_id]
                                    for ask in masks
                                ],
                                dtype="float64",
                                ndmin=2,
                    ...
                    for diff_message in replay_diffs:
                        ...
                        if len(mbids) > 0:
                            if isinstance(mbids[0], list):
                                np_bids = np.array(
                                    [
                                        [diff_message.timestamp, float(bid[0]), float(bid[1]), diff_message.update_id]
                                        for bid in mbids
                                    ],
                                    dtype="float64",
                                    ndmin=2,
                        ...
                        if len(masks) > 0:
                            if isinstance(mbids[0], list):
                                np_asks = np.array(
                                    [
                                        [diff_message.timestamp, float(ask[0]), float(ask[1]), diff_message.update_id]
                                        for ask in masks
                                    ],
                                    dtype="float64",
                                    ndmin=2,
