...
from hummingbot.connector.exchange.southxchange import (
    southxchange_constants as CONSTANTS,
    southxchange_utils,
    southxchange_web_utils as web_utils,
)
...
class SouthXchangeExchange(ExchangePyBase):
    ...
    def __init__(
        ...
        SouthXchange_api_secret: str,
        trading_pairs: Optional[List[str]] = None,
        trading_required: bool = True,
        domain: str = CONSTANTS.DEFAULT_DOMAIN,
    ):
        self.api_key = SouthXchange_api_key
        self.secret_key = SouthXchange_api_secret
        self._domain = domain
        self._trading_required = trading_required
        self._trading_pairs = trading_pairs
        self._last_trades_poll_SouthXchange_timestamp = 1.0
        super().__init__(client_config_map)

    @staticmethod
    def SouthXchange_order_type(order_type: OrderType) -> str:
        return order_type.name.upper()

    @staticmethod
    def to_hb_order_type(SouthXchange_type: str) -> OrderType:
        return OrderType[SouthXchange_type]

    @property
    def authenticator(self):
        return SouthXchangeAuth(api_key=self.api_key, secret_key=self.secret_key, time_provider=self._time_synchronizer)
    ...
    def name(self) -> str:
        if self._domain == "com":
            ...
            return f"SouthXchange_{self._domain}"
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return SouthXchangeAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return SouthXchangeAPIUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory,
    ...
    async def _place_order(
        ...
        **kwargs,
        ...
        type_str = SouthXchangeExchange.SouthXchange_order_type(order_type)
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(southxchange_utils.is_exchange_information_valid, trading_pair_rules):
            try:
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://github.com/SouthXchange-exchange/SouthXchange-official-api-docs/blob/master/user-data-stream.md
                # As per the order update section in SouthXchange the ID of the order being canceled is under the "C" key
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case SouthXchange's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since SouthXchange's get order endpoint does not return trade IDs.
        ...
        if long_interval_current_tick > long_interval_last_tick or (
            ...
            query_time = int(self._last_trades_poll_SouthXchange_timestamp * 1e3)
            self._last_trades_poll_SouthXchange_timestamp = self._time_synchronizer.time()
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(southxchange_utils.is_exchange_information_valid, exchange_info["symbols"]):
