...
class XeggexExchange(ExchangePyBase):
    ...
    def authenticator(self):
        return XeggexAuth(api_key=self.api_key, secret_key=self.secret_key, time_provider=self._time_synchronizer)
                    ...
                    if reportType == "trade":
                        ...
                        if tracked_order is not None:
                            fee = TradeFeeBase.new_spot_fee(
                                ...
                                flat_fees=[
                                    TokenAmount(amount=Decimal(message_params.get("tradeFee")), token=quote_asset)
                                ],
                            )
                            trade_update = TradeUpdate(
                                ...
                                fill_quote_amount=Decimal(message_params["executedQuantity"])
    ...
    async def _update_order_fills_from_trades(self):
        ...
        if long_interval_current_tick > long_interval_last_tick or (
            ...
            for trading_pair in trading_pairs:
                params = {
                    "symbol": str(await self.exchange_symbol_associated_to_pair(trading_pair=trading_pair)).replace(
                        "/", "_"
                    )
                }
                ...
                tasks.append(self._api_get(path_url=url_path, params=params, is_auth_required=True))
                    ...
                    if exchange_order_id in order_by_exchange_id_map:
                        ...
                        self.trigger_event(
                            ...
                            OrderFilledEvent(
                                ...
                                order_type=(
                                    OrderType.LIMIT_MAKER if trade["side"] != trade["triggeredBy"] else OrderType.LIMIT
                                ),
                                price=Decimal(trade["price"]),
                                amount=Decimal(trade["quantity"]),
                                trade_fee=DeductedFromReturnsTradeFee(
                                    flat_fees=[TokenAmount(quote_asset, Decimal(trade["fee"]))]
                                ),
    ...
    async def _update_balances(self):
        ...
        balances = await self._api_get(path_url=CONSTANTS.USER_BALANCES_PATH_URL, is_auth_required=True)
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        ...
        for symbol_data in filter(xeggex_utils.is_market_active, exchange_info):
            ...
            mapping[symbol] = combine_to_hb_trading_pair(base=symbol_data["primaryTicker"], quote=symbol.split("/")[1])
